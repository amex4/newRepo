  ...
  return ( <>
    <div className="mt-6">
        <Link to="/superadmin/add">
            <button className="bg-blue-600 w-32 mb-6 mt-6 ml-20">Add An Item</button>
        </Link>
    </div>
    { products ?
     <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
       {products.map((product) => (
         <div key={product._id} className="group relative">
           ...
           <div>
             <button  className=" mt-5 w-32 bg-orange-600" onClick={() => handleClick(product)}>delete</button>
           </div>
         </div>
       ))}
     </div>
